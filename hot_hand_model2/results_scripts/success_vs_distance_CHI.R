...
sigmab <- as.vector(unlist(MCMCchains(rsamps_hot_hand_CHI, 
                                      params = 'sigmab', ISB = F)))

betad <- as.vector(unlist(MCMCchains(rsamps_hot_hand_CHI, 
                                     params = 'beta_d', ISB = F)))
...
polygon(c(distance,rev(distance)), 
        ...
        col = alpha("#440154FF",0.8), lwd=4, border=NA)
...
polygon(c(distance,rev(distance)), 
        ...
        col = alpha("#FDE725FF",0.8), lwd=4, border=NA)
