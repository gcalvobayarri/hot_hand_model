...
library(boot)
library(expm)
source('./script/CPO/posterior_draws_HMM_CHI.R') #saved in draws_posterior
...
log_cpo_i <- function(data_i, draws, log = TRUE) { 
  ...
  for(i in 1 : dim(draws)[1]){
    Gamma <- matrix(    
      c(1 - draws[i, 89 + data_i$match_id], draws[i, 89 + data_i$match_id],
        draws[i, 173 + data_i$match_id], 1 -  draws[i, 173 + data_i$match_id]),
      nrow = 2, byrow = T) # 
...
log_cpo_HMM_CHI <- log_cpo


# hist(log_cpo)


save(log_cpo_HMM_CHI, 
     file = './results/log_cpo_HMM_CHI.RData')
