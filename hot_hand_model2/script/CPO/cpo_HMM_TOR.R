...
library(boot)
library(expm)
source('./script/CPO/posterior_draws_HMM_TOR.R') #saved in draws_posterior

source('./script/CPO/data_i_HMM_TOR.R')
...
log_cpo_i <- function(data_i, draws, log = TRUE) { 
  ...
  for(i in 1 : dim(draws)[1]){
    Gamma <- matrix(    
      c(1 - draws[i, 87 + data_i$match_id], draws[i, 87 + data_i$match_id],
        draws[i, 169 + data_i$match_id], 1 -  draws[i, 169 + data_i$match_id]),
      nrow = 2, byrow = T) # 
...
log_cpo_HMM_TOR <- log_cpo


# hist(log_cpo)


save(log_cpo_HMM_TOR, 
     file = './results/log_cpo_HMM_TOR.RData')
