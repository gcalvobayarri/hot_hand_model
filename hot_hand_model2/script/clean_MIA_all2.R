...
save(shots_MIA, file = './data/shots_MIA_all2.RData')
...
games <- sqldf('SELECT COUNT(game_id), match_id FROM data_shots_MIA GROUP BY game_id
               ORDER BY match_id')
...
save(matrix_tiros, ft, distance_matrix, tiros_por_partido, 
     file = './data/matrix_data_ft_complete_season_all2.RData')
